    ...
    public partial class WebForm19 : System.Web.UI.Page
    {
        ...
        private void ResetUserPassword(string email)
        {
            ...
            if (Common.SendEmail(emailContent, emailSubject, toEmail, fromEmail, password))
            {
                ...
                {
                    conn.Open();
                    SqlCommand cmd = new SqlCommand("update students set Password = '" + Common.ComputeSha256Hash(randPass) + "' where Email = '" + email + "';");
                    cmd.ExecuteNonQuery();
                }
            }
            else
        ...
        {
            Response.Redirect("Signup.aspx?type=Student");
        }
    }
}
